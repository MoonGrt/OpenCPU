...
class SerialConfig(QDialog):
    ...
    def get_config(self):
        ...
        return {
            ...
            'stop_bit': self.stop_bit_combo_box.currentText()
        }
    ...
    def refresh_ports(self):
        ...
        # 将端口添加到下拉框中
        for port in ports:
            self.com_line_edit.addItem(port.portName())


if __name__ == '__main__':
    app = QApplication(sys.argv)
    serial_dialog = SerialConfig()
    serial_dialog.exec_()

    # 在窗口关闭后获取用户选择的配置
    print(serial_dialog.get_config())
    sys.exit()