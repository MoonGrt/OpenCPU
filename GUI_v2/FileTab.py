import sys
from PyQt5.QtWidgets import  QVBoxLayout, QSplitter, QTableWidgetItem, QTableWidget, QComboBox
...
class FileTab(QWidget):
    def __init__(self, parent=None):
        ...
        self.table_left.itemChanged.connect(self.handle_left_change)  # 实时监控左侧变化
        ...
        # self.table_right.itemChanged.connect(self.handle_right_change)  # 实时监控右侧变化
    ...
    def set_row(self, table, row, code='', opcode='', rd='', rs='', imm='', IMM=''):
        """向表格中填入一行数据"""
        if code:
            columns = code.split()  # 用空格分割文本为列
            opcode = columns[0]
        ...
        opcode_combo.addItems([""] + list(self.Assembler.opcode_dict.keys()))  # 从字典中添加引脚作为选项
        opcode_combo.setCurrentText(opcode)
        opcode_combo.currentIndexChanged.connect(lambda: self.opcode_changed(row))
        self.table_right.setItem(row, 0, QTableWidgetItem(self.code_decode(opcode)))
        table.setCellWidget(row, 0, opcode_combo)
        
        if opcode == 'rc':
            # 第二三四列：空
            table.setSpan(row, 1, 1, 3)  # 合并列  setSpan(self, row: int, column: int, rowSpan: int, columnSpan: int)
            empty_item = QTableWidgetItem("")
            empty_item.setFlags(Qt.ItemIsEnabled)  # 不允许编辑或选择
            ...
            # 第二列：rd
            rd_combo = QComboBox()
            rd_combo.addItems(list(self.Assembler.reg_dict.keys()))  # 从字典中添加引脚作为选项
            rd_combo.setCurrentText(rd)
            rd_combo.currentIndexChanged.connect(lambda: self.reg_changed(row, 1))
            self.table_right.setItem(row, 1, QTableWidgetItem(self.code_decode(rd)))
            table.setCellWidget(row, 1, rd_combo)
            # 第三四列：IMM
            table.setSpan(row, 2, 1, 2)  # 合并列  setSpan(self, row: int, column: int, rowSpan: int, columnSpan: int)
            table.setItem(row, 2, QTableWidgetItem(IMM))  # 添加一个空选项
        else:
            if code:
                rd = columns[1]
                rs = columns[2]
            ...
            # 第三列：rs/IMM
            rs_combo = QComboBox()
            rs_combo.addItems(list(self.Assembler.reg_dict.keys()))  # 从字典中添加引脚作为选项
            rs_combo.setCurrentText(rs)
            rs_combo.currentIndexChanged.connect(lambda: self.reg_changed(row, 2))
            self.table_right.setItem(row, 2, QTableWidgetItem(self.code_decode(rs)))
            table.setCellWidget(row, 2, rs_combo)
            # 第四列：imm/IMM
            table.setItem(row, 3, QTableWidgetItem(imm))

    def opcode_changed(self, row):
        ComboBox = self.table_left.cellWidget(row, 0)
        if isinstance(ComboBox, QComboBox):
            opcode = ComboBox.currentText()
            self.set_row(self.table_left, row, opcode=opcode)
            self.table_right.setItem(row, 0, QTableWidgetItem(self.code_decode(opcode)))

    def reg_changed(self, row, col):
        ComboBox = self.table_left.cellWidget(row, col)
        if isinstance(ComboBox, QComboBox):
            reg = ComboBox.currentText()
            self.table_right.setItem(row, col, QTableWidgetItem(self.code_decode(reg)))
    ...
    def text_to_table(self):
        ...
        for row, line in enumerate(lines_left):
            self.set_row(self.table_left, row, code=line)
    ...
    def handle_left_change(self, item):
    ...
    def handle_right_change(self, item):
        if not self.table_syncing:  # 避免循环调用
            self.table_syncing = True
